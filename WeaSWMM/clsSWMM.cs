...
namespace NCEIData
{
    ...
    //}
    //public class SWMMPoint
    //{
    //    public double X { get; set; }
    //    public double Y { get; set; }
    //}
    public class clsSWMM
    {
        ...
        public clsSWMM(frmSWMM _fSWMM, IMap _map, string _wdmFile)
        {
            WDMFile = _wdmFile;
            appMap = _map;
            fSWMM = _fSWMM;
            dictGages = fSWMM.dictGages;
            //lstOfPoints = fSWMM.lstOfPoints;
            dictPoints = fSWMM.dictPoints;
            ...
            //init dates
            //MinDate = fSWMM.dtBegDate.MinDate;
            //MaxDate = fSWMM.dtEndDate.MaxDate;
            SimBegDate = fSWMM.dtBegDate.Value;
            SimEndDate = fSWMM.dtEndDate.Value;
        ...
        public bool GetWDMAttributes()
        {
            ...
            try
            {
                ...
                fSWMM.dtBegDate.Value = MinDate;
                fSWMM.dtEndDate.Value = MaxDate;

                fSWMM.dtBegDate.MinDate = MinDate;
                fSWMM.dtBegDate.MaxDate = MaxDate.AddYears(-1);
                fSWMM.dtEndDate.MinDate = MinDate.AddYears(1);
                fSWMM.dtEndDate.MaxDate = MaxDate;
                fSWMM.dictGages = dictGages;
            }
            catch (Exception ex)
            {
                return false;
        ...
        private bool GetDictionaryOfDatasets()
        {
            ...
            try
            {
                ...
                foreach (atcTimeseries lDataSet in lwdm.DataSets)
                {
                    ...
                    if (lstVars.Contains(svar) && tunit.Contains("Hour"))
                    {
        ...
        public bool AssignMetVariableByBasin()
        {
            ...
            try
            {
                ...
                int icount = 0;
                Debug.WriteLine("Basin Count = " + nbasin.ToString());
                ...
                foreach (KeyValuePair<int, CPoint> kv in BasinCentroid)
                {
                    ...
                    Debug.WriteLine("Assigning met stations to selected location " + basin.ToString() + "(" +
                        icount.ToString() + " of " + nbasin.ToString() + ")");
        ...
        {
            Debug.WriteLine("Entering AssignMetStationVar");
            List<string> lstDSN = new List<string>();
