...
namespace WeaEFDC
{
    class clsEFDC
    {
        ...
        private List<string> EFDCVars;
        private frmEFDC fEFDC;
        ...
        public clsEFDC(frmEFDC _fEFDC, IMap _map, string _wdmFile)
        {
            WDMFile = _wdmFile;
            appMap = _map;
            fEFDC = _fEFDC;
            dictGages = fEFDC.dictGages;
            dictPoints = fEFDC.dictPoints;
            EFDCVars = fEFDC.EFDCVars;
            ...
            //init dates
            //MinDate = fEFDC.dtBegDate.MinDate;
            //MaxDate = fEFDC.dtEndDate.MaxDate;
            SimBegDate = fEFDC.dtBegDate.Value;
            SimEndDate = fEFDC.dtEndDate.Value;
        ...
        private void InitializeMetTable()
        {
            ...
            foreach (string s in EFDCVars)
                MetTable.Columns.Add(s, typeof(string));
        }
        public bool GetSubbasinCentroid()
        {
            //for EFDC these are mapped selected points or from lat-lon file
        ...
        private bool GetDictionaryOfDatasets()
        {
            ...
            try
            {
                ...
                //iterate for all datasets, only select hourly data
                foreach (atcTimeseries lDataSet in lwdm.DataSets)
                {
                    ...
                    fEFDC.WriteStatus("Reading attributes of " + sloc + ":" +
                        svar + "(" + ncnt.ToString() + " of " + numdset.ToString() + ")");

                    //add to dictionary of gages, check only period of series
                    //when doing assignment, and time unit is hourly
                    if (EFDCVars.Contains(svar) && tunit.Contains("Hour"))
                    {
        ...
        public bool AssignMetVariableByBasin()
        {
            ...
            try
            {
                ...
                foreach (KeyValuePair<int, CPoint> kv in BasinCentroid)
                {
                    ...
                    fEFDC.WriteStatus("Assigning met stations to selected location " + basin.ToString() + "(" +
        ...
        private int AssignMetStationVar(string svar, CPoint cpt)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    errmsg += crlf + "Or period of record is not within simulation period!";
                    WriteMessage("Error!", errmsg);
                    mxDSN = 0;
                }
            }
        ...
        {
            fEFDC.statuslbl.Text = msg;
            fEFDC.statusStrip.Refresh();
        }
