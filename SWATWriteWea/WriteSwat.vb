...
Public Class WriteSwat
    ...
    Public Sub New(ByRef _statuslbl As ToolStripLabel,
        ...
        'debug check dates
        Dim dt As Date = Date.FromOADate(aDateStart)
        Debug.WriteLine("Date Beg = " + pDateStart.ToString() + ", " + dt.ToString())
        dt = Date.FromOADate(aDateEnd)
        Debug.WriteLine("Date End = " + pDateEnd.ToString() + ", " + dt.ToString())

        WriteSWATMetFiles(lMetWDMFile, aSaveInFolder, aDateStart, aDateEnd, aListDSN, aLocation)
    End Sub
    ...
    Public Sub WriteSWATMetFiles(ByVal lMetWDMFile As String,
        ...
        Debug.WriteLine("Executing WriteSWATMetFiles ...")
        ...
        If lMetWDM.Open(lMetWDMFile) Then
            ...
            If lMetWDM.DataSets.Count > 0 Then
                WriteSwatMetInput(lMetWDM, aSaveInFolder, aDateStart, aDateEnd, aLocation)
            Else
    ...
    Public Sub WriteSwatMetInput(ByVal aMetWDM As atcDataSource,
        ...
        Debug.WriteLine("Executing WriteSWATMetInput ...")
        Dim aWeaFile As String = String.Empty
        Dim aDSN As Integer = 0

        'write PREC
        WriteStatus("Writing PREC for location " + aLocation)
        ...
        'write PEVT
        WriteStatus("Writing PEVT for location " + aLocation)
        ...
        'write ATEM
        WriteStatus("Writing ATEM for location " + aLocation)
        ...
        'write SOLR
        WriteStatus("Writing SOLR for location " + aLocation)
        ...
        'write WIND
        WriteStatus("Writing WIND for location " + aLocation)
        ...
        'write DEWP
        WriteStatus("Writing DEWP for location " + aLocation)
        ...
        'write CLOU
        WriteStatus("Writing CLOU for location " + aLocation)
    ...
    Private Function WritePCP(ByVal aWDM As atcDataSourceWDM,
        ...
        Debug.WriteLine("Executing WritePCP ...")
        Dim lCons As String = "PREC"
        Try
            Dim lTS As atcTimeseries = GetWDMTimeseries(aWDM, aDSN, lCons)
            Debug.WriteLine("NumValues of PREC = " + lTS.numValues.ToString())
    ...
    Private Function WritePCPSeries(ByVal aTS As atcTimeseries,
        ...
        Try
            ...
            End If
        Catch ex As Exception
            errmsg = "Error writing PREC timeseries" & vbCrLf & ex.Message & ex.StackTrace
    ...
    Private Function WriteTMP(ByVal aWDM As atcDataSourceWDM,
                              ...
                              ByVal aDSN As Integer,
                              ByVal aDateStart As Double, ByVal aDateEnd As Double) As Boolean
        Debug.WriteLine("Executing WriteTMP ...")
        Dim lCons As String = "ATEM"
        Dim lTS As atcData.atcTimeseries = GetWDMTimeseries(aWDM, aDSN, lCons)
        Debug.WriteLine("NumValues Daily ATEM = " + lTS.numValues.ToString())
    ...
    Private Function WriteTMPSeries(ByVal aTS As atcTimeseries,
                              ...
                              ByVal aDateStart As Double,
                              ByVal aDateEnd As Double) As Boolean
        Debug.WriteLine("Executing WriteTMPSeries ...")
        Try
            If aTS IsNot Nothing Then
                aTS.EnsureValuesRead()
                Debug.WriteLine("NumValues Daily ATEM = " + aTS.numValues.ToString())
        ...
        Catch ex As Exception
            errmsg = "Error writing TMP timeseries" & vbCrLf & ex.Message & ex.StackTrace
            MessageBox.Show(errmsg, "Error!", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return False
        End Try
        Return True
    ...
    Private Sub WriteTMPline(ByVal aWriter As IO.StreamWriter, ByVal aDate As Date, ByVal aMax As Double, ByVal aMin As Double)
        If (Date.Compare(aDate, pDateStart) >= 0 And
            Date.Compare(aDate, pDateEnd) <= 0) Then
            If aMax < aMin Then
                aMax = -99
                aMin = -99
            Else
    ...
    Private Function WriteSLR(ByVal aWDM As atcDataSourceWDM,
                              ...
                              ByVal aDSN As Integer,
                              ByVal aDateStart As Double, ByVal aDateEnd As Double) As Boolean
        Debug.WriteLine("Executing WriteSLR ...")
        Dim lCons As String = "SOLR" '"DSOL"
        Dim lTS As atcData.atcTimeseries = GetWDMTimeseries(aWDM, aDSN, lCons)
        Debug.WriteLine("NumValues Daily SOLR = " + lTS.numValues.ToString())
    ...
    Private Function WriteSLRSeries(ByVal aTS As atcTimeseries,
                              ...
                              ByVal aDateStart As Double,
                              ByVal aDateEnd As Double) As Boolean
        Debug.WriteLine("Executing WriteSLRSeries ...")
        Try
            If aTS IsNot Nothing Then
                aTS.EnsureValuesRead()
                Debug.WriteLine("NumValues Daily SOLR = " + aTS.numValues.ToString())
        ...
        Catch ex As Exception
            errmsg = "Error writing SOLR timeseries" & vbCrLf & ex.Message & ex.StackTrace
            MessageBox.Show(errmsg, "Error!", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return False
        End Try
        Return True
    ...
    Private Function WriteWND(ByVal aWDM As atcDataSourceWDM,
                              ...
                              ByVal aDSN As Integer,
                              ByVal aDateStart As Double, ByVal aDateEnd As Double) As Boolean
        Debug.WriteLine("Executing WriteWND ...")
        Dim lCons As String = "WIND"
        Dim lTS As atcData.atcTimeseries = GetWDMTimeseries(aWDM, aDSN, lCons)
        Debug.WriteLine("NumValues Daily WIND = " + lTS.numValues.ToString())
    ...
    Private Function WriteWNDSeries(ByVal aTS As atcTimeseries,
        ...
        Try
            If aTS IsNot Nothing Then
                aTS.EnsureValuesRead()
                aTS = atcData.modTimeseriesMath.Aggregate(aTS, atcTimeUnit.TUDay, 1, atcTran.TranAverSame)
                Debug.WriteLine("NumValues Daily WIND = " + aTS.numValues.ToString())
        ...
        Catch ex As Exception
            errmsg = "Error writing WIND timeseries" & vbCrLf & ex.Message & ex.StackTrace
            MessageBox.Show(errmsg, "Error!", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return False
        End Try
        Return True
    ...
    Private Function WritePET(ByVal aWDM As atcDataSourceWDM,
                              ...
                              ByVal aDSN As Integer,
                              ByVal aDateStart As Double, ByVal aDateEnd As Double) As Boolean
        Debug.WriteLine("Executing WritePET ...")
        Dim lCons As String = "PEVT"
        Dim lTS As atcData.atcTimeseries = GetWDMTimeseries(aWDM, aDSN, lCons)
        Debug.WriteLine("NumValues Daily PET = " + lTS.numValues.ToString())
    ...
    Private Function WritePETSeries(ByVal aTS As atcTimeseries,
        ...
        Try
            If aTS IsNot Nothing Then
                aTS.EnsureValuesRead()
                aTS = atcData.modTimeseriesMath.Aggregate(aTS, atcTimeUnit.TUDay, 1, atcTran.TranSumDiv)
                Debug.WriteLine("NumValues Daily PET = " + aTS.numValues.ToString())
        ...
        Catch ex As Exception
            errmsg = "Error writing PEVT timeseries" & vbCrLf & ex.Message & ex.StackTrace
            MessageBox.Show(errmsg, "Error!", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return False
        End Try
        Return True
    ...
    Private Function WriteDEW(ByVal aWDM As atcDataSourceWDM,
        ...
        Debug.WriteLine("Executing WriteDEW ...")
        Dim lCons As String = "DEWP"
        Dim lTS As atcData.atcTimeseries = GetWDMTimeseries(aWDM, aDSN, lCons)
        If lTS IsNot Nothing Then
            lTS.EnsureValuesRead()
            Debug.WriteLine("NumValues Daily DEWP = " + lTS.numValues.ToString())
    ...
    Private Function WriteCLO(ByVal aWDM As atcDataSourceWDM,
        ...
        Debug.WriteLine("Executing WriteCLO ...")
        Dim lCons As String = "CLOU"
        Dim lTS As atcData.atcTimeseries = GetWDMTimeseries(aWDM, aDSN, lCons)
        If lTS IsNot Nothing Then
            lTS = atcData.modTimeseriesMath.Aggregate(lTS, atcTimeUnit.TUDay, 1, atcTran.TranAverSame)
            Debug.WriteLine("NumValues Daily CLOU = " + lTS.numValues.ToString())
