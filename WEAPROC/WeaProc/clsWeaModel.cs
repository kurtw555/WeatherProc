...
namespace NCEIData
{
    class clsWeaModel
    {
        ...
        private int tstep, nhar = 12, nhfit = 2, ndiv = 2, nhperiod = 24;
        ...
        public clsWeaModel(frmMain _fMain, string _site, string _svar,
        ...
        public void FitLinearModel(int timestep)
        {
            switch (timestep)
            {
                case (int)Interval.Hourly:
                    nhar = 12; nhfit = 2; ndiv = 2; nhperiod = 24;
                    ...
                    switch (IModelType)
                    {
                        case (int)ModelType.Seasonal:
                            ...
                            moments = CalculateSeasonalHourlyMoments(dseries);
                            model = CalculateSeasonalHourlyHarmonics();
        ...
        private bool DifferenceSeries(SortedDictionary<DateTime, string> tseries)
        {
            try
            {
                fMain.WriteLogFile("Generating difference series " + svar + ": " + site);
            ...
            {
                nceimsg.ShowError("Error calculating serial correlation of difference hourly series!", ex);
                return false;
            }
        }
        #region "Validation"
        ...
        {
            fMain.WriteLogFile("Calculating seasonal hourly mean and variance for " + svar + ": " + site);
            ...
            foreach (var kv in calc)
            {
                ...
                //Tuple<double, double> moment = Statistics.MeanStandardDeviation(lstdat);
                List<double> lst = new List<double>();
                //lst.Add(moment.Item1); lst.Add(moment.Item2);
                ix++;
                //debug
                //Debug.WriteLine("{0},{1},{2},{3}", kv.Key.ToString(), ix.ToString(), 
                //    moment.Item1.ToString("F4"), moment.Item2.ToString("F4"));
        ...
        private SortedDictionary<string, double> CalculateSeasonalHourlyHarmonics()
        {
            ...
            fMain.WriteLogFile("Calculating monthly periodicities of hourly mean and variance for " + svar + ": " + site);
            try
            {
                SortedDictionary<string, double> mdl = new SortedDictionary<string, double>();

                int nh = nhar;  //nhar is 12, half of daily period of 24
                ...
                for (int imo = 1; imo <= 12; imo++)
                {
                    ...
                    for (int jhr = 0; jhr < 24; jhr++)
                    {
                        ...
                        moments.TryGetValue(jmohr, out parms);
                        lstParms.Add(parms);
                        parms = null;
                    }
            ...
            {
                nceimsg.ShowError("Error calculating periodicities in hourly mean and variance!", ex);
                return null;
            }
        }
        private SortedDictionary<Int32, List<double>> GeneratePeriodicHourlyMoments()
        {
            ...
            {
                nceimsg.ShowError("Error generating periodic hourly means and variances!", ex);
                return null;
            }
        }
        private bool StandardizeSeasonalHourly()
        {
            ...
            {
                nceimsg.ShowError("Error calculating standardized hourly series!", ex);
                return false;
            }
        }
        private bool AutoCorrelationHourly()
        {
            ...
            {
                nceimsg.ShowError("Error calculating serial correlation of standardized hourly series!", ex);
                return false;
            }
        ...
        {
            fMain.WriteLogFile("Calculating seasonal daily mean and variance for " + svar + ": " + site);
            ...
            //feb 29
            List<double> l229 = new List<double>();
            calc.TryGetValue(229, out l229);
            ...
            try
            {
                ...
                foreach (var kv in calc)
                {
                    ...
                    ix++;
                    List<double> lst = new List<double>();
                    //if (double.IsNaN(moment.Item2) || double.IsInfinity(moment.Item2))
                    //{
                    //    List<double> prev = new List<double>();
                    //    mom.TryGetValue(prekey, out prev);
                    //    lst.Add(prev.ElementAt(0));
                    //    lst.Add(prev.ElementAt(1));
                    //    prev = null;
                    //}
                    //else
                    //{
                ...
                //use mar 1 for feb 29
                //List<double> lstm = new List<double>();
                //mom.TryGetValue(301, out lstm);
                //mom.Remove(229);
                //mom.Add(229, lstm);
                //lstm = null;
            }
            ...
            {
                nceimsg.ShowError("Error calculating seasonal daily moments!", ex);
                return null;
            }
            return mom;
        }
        private SortedDictionary<string, double> CalculateSeasonalDailyHarmonics()
        {
            ...
            fMain.WriteLogFile("Calculating monthly periodicities of daily mean and variance for " + svar + ": " + site);
            try
            {
                SortedDictionary<string, double> mdl = new SortedDictionary<string, double>();
            ...
            {
                nceimsg.ShowError("Error calculating periodicities in daily mean and variance!", ex);
                return null;
            }
        }
        private SortedDictionary<Int32, List<double>> GeneratePeriodicDailyMoments()
        {
            ...
            {
                nceimsg.ShowError("Error generating periodic daily means and variances!", ex);
                return null;
            }
        }
        ...
        private bool AutoCorrelationDaily()
        {
            ...
            {
                nceimsg.ShowError("Error calculating serial correlation of standardized daily series!", ex);
                return false;
            }
