...
namespace NCEIData
{
    ...
    public class SWATPoint
    {
        public double X { get; set; }
        public double Y { get; set; }
    }

    public class clsSWAT
    {
        ...
        private SortedDictionary<int, SWATPoint> BasinCentroid;
        private DataTable MetTable;
        private List<string> lstVars = new List<string>() { "PREC", "PEVT" };
        private frmSWAT fSWAT;
        string errmsg = string.Empty;
        string crlf = Environment.NewLine;
        DateTime MinDate, MaxDate;
        DateTime SimBegDate, SimEndDate;
        private List<SWATPoint> lstOfPoints;
        private Dictionary<string, SWATPoint> dictPoints;

        public clsSWMM(frmSWAT _fSWAT, IMap _map, string _wdmFile)
        {
            WDMFile = _wdmFile;
            appMap = _map;
            fSWAT = _fSWAT;
            dictGages = fSWAT.dictGages;
            //lstOfPoints = fSWMM.lstOfPoints;
            dictPoints = fSWAT.dictPoints;

            //initialize dictionaries of gages and basin centroid
            dictGages = new Dictionary<string, SortedDictionary<int, clsStation>>();
            BasinCentroid = new SortedDictionary<int, SWATPoint>();

            mapProjection = appMap.Projection;
            WDMFileName = Path.GetFileName(WDMFile);

            //init dates
            MinDate = fSWAT.dtBegDate.MinDate;
            MaxDate = fSWAT.dtEndDate.MaxDate;
            SimBegDate = fSWAT.dtBegDate.Value;
            SimEndDate = fSWAT.dtEndDate.Value;
        ...
        public bool GetSubbasinCentroid()
        {
            ...
            try
            {
                ...
                {
                    SWATPoint pt = dictPoints.Values.ElementAt(i);
        ...
        public bool GetWDMAttributes()
        {
            Cursor.Current = Cursors.WaitCursor;
            try
            {
                if (!GetDictionaryOfDatasets())
                    return false;
                //debug
                foreach(KeyValuePair<string, SortedDictionary<int,clsStation>>kv in dictGages)
                {
                    Debug.WriteLine("kv.key in dict=" + kv.Key);
                }

                fSWAT.dtBegDate.Value = MinDate;
                fSWAT.dtEndDate.Value = MaxDate;

                fSWAT.dtBegDate.MinDate = MinDate;
                fSWAT.dtBegDate.MaxDate = MaxDate.AddYears(-1);
                fSWAT.dtEndDate.MinDate = MinDate.AddYears(1);
                fSWAT.dtEndDate.MaxDate = MaxDate;
            }
            catch (Exception ex)
            {
                return false;
        ...
        private bool GetDictionaryOfDatasets()
        {
            ...
            try
            {
                ...
                foreach (atcTimeseries lDataSet in lwdm.DataSets)
                {
                    ...
                    fSWMM.WriteStatus("Reading attributes of " + sloc + ":" +
        ...
        public bool AssignMetVariableByBasin()
        {
            try
            {
                ...
                foreach (KeyValuePair<int, SWMMPoint> kv in BasinCentroid)
                {
                    icount++;
                    int dsn = 0;
                    int basin = kv.Key;
                    string gage;
                    SWMMPoint cpt = kv.Value;
                    dictDsn = new Dictionary<string, string>();

                    fSWMM.WriteStatus("Assigning met stations to selected location " + basin.ToString() + "(" +
        ...
        /// subbasin centroid
        /// <returns></returns>
        private int AssignMetStationVar(string svar, SWMMPoint cpt)
        {
            List<string> lstDSN = new List<string>();
        ...
        private double GetDistanceToBasin(double x, double y, SWMMPoint cpt)
        {
            double dist = 0.0;
        ...
        {
            fSWMM.statuslbl.Text = msg;
            fSWMM.statusStrip.Refresh();
        }
