...
namespace WeaWASP
{
    public partial class frmWASP : Form
    {
        ...
        //dictionary of gages keyed on station
        ...
        private List<string> WeaVars = new List<string>()
              { "pcp", "tmp", "slr", "wnd", "dew", "clo" };
        public List<string> WASPVars = new List<string>()
              { "PREC", "ATEM", "SOLR", "WIND", "DEWP", "CLOU" };
        private bool showForm = true;

        public frmWASP(Map _map, string _wdmFile, string _sdbFile, List<CPoint> _lstOfPoints)
        {
            InitializeComponent();
            ...
            lblBegDate.Text = dtBegDate.Value.ToShortDateString();
            lblEndDate.Text = dtEndDate.Value.ToShortDateString();
            ...
            //assign nearest stations
            //AssignNearestStations();
        }
        public bool ShowFormWASP()
        {
            return showForm;
        }
        ...
        {
            cWASP = new clsWASP(this, appMap, WDMFile);
            if (!cWASP.GetSubbasinCentroid())
            {
                cWASP = null;
                return;
            }
            ...
            if (cWASP.AssignMetVariableByBasin())
            {
                MetTable = cWASP.WeatherTable();
                //cWASP = null;
                ...
                btnClose.Enabled = true;
                btnAssign.Text = "Write WASP Weather File(s)";
                grpLocation.Text = "WASP Weather Locations (" +
                MetTable.Rows.Count.ToString() + " stations)";
            }
        }
        private List<string> GetUniqueDSNs()
        {
            //iterate on met tables rows, get all uniques svr-dsn's
            ...
            foreach (string svar in WASPVars)
            {
                ...
                foreach (DataRow drow in MetTable.Rows)
                {
                    ...
                    string vardsn = svar + ":" + wea.ToString();
                    if (!lstWeaDSN.Contains(vardsn))
                        lstWeaDSN.Add(vardsn);
                }
            }
            return lstWeaDSN;
            //WriteWASPWeatherFiles(lstWeaDSN);
            //lstWeaDSN = null;
        }
        private void btnAssign_Click(object sender, EventArgs e)
        {
            ...
            switch (btnAssign.Text)
            {
                ...
                case "Write WASP Weather File(s)":

                    Cursor.Current = Cursors.WaitCursor;
                    List<string> lstSelectedDSN = GetUniqueDSNs();
                    int nseries = UploadWASPWeatherFiles(lstSelectedDSN);
                    lstSelectedDSN = null;
                    Cursor.Current = Cursors.Default;

                    string mesg = "Uploaded WASP weather series for " + nseries.ToString() + " locations.";
                    MessageBox.Show(mesg, "Information!", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    break;
            }
            cWASP = null;
        }
        private int UploadWASPWeatherFiles(List<string> lstWeaDSN)
        {
            int nseries = 0;
            try
            {
                ...
                nseries = lstWeaDSN.Count;
                clsWriteWASP wriWASP = new clsWriteWASP(this, WDMFile, SDBFile, lstWeaDSN, dtBeg, dtEnd, dictGages);
                wriWASP.UploadWASPWeather();
                wriWASP = null;
            }
            catch (Exception ex)
            {
        ...
        private List<string> SetupWASPWeatherFiles(string svar)
        {
            Cursor.Current = Cursors.WaitCursor;
            List<string> lstWeaDsn = new List<string>();
            try
        ...
        private void btnFolder_Click(object sender, EventArgs e)
        {
            ...
            using (FolderBrowserDialog openFD = new FolderBrowserDialog())
            {
                ...
                openFD.Description = "Select folder to save WASP weather files ...";
