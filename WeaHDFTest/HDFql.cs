...
public class HDFql {
		...
		private static int VariableRegister(object variable, int variableTransient)
		{
			...
			if (type.IsArray == false)   // INFO: only arrays are allowed (otherwise it is not possible to have a "real" reference of the object being registered)
			{
				return ErrorUnexpectedDataType;
			}
			dataType = type.GetElementType();
			if (dataType != typeof(sbyte) && dataType != typeof(byte) && dataType != typeof(short) && dataType != typeof(ushort) && dataType != typeof(int) && dataType != typeof(uint) && dataType != typeof(long) && dataType != typeof(ulong) && dataType != typeof(float) && dataType != typeof(double) && type.IsValueType == false && type.IsEnum == true)   // INFO: no need to test for build-in types since in C sharp primitive types (e.g. sbyte) are aliases for build-in types (e.g. SByte)
			{
				return ErrorUnexpectedDataType;
			}
			number = -1;
